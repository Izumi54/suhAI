...
class WeatherBackend {
    ...
    async getWeatherData(location) {
        const response = await fetch(
            `https://api.weatherapi.com/v1/current.json?key=${this.API_KEY}&q=${location}&aqi=no`
        );
    ...
    updateWeatherDisplay(data) {
        ...
        // Update temperature
        const tempElements = document.querySelectorAll('#currentTemp, #currentTempMobile');
        ...
        // Update feels like
        const feelsLikeElements = document.querySelectorAll('#feelsLike, #feelsLikeMobile');
        feelsLikeElements.forEach(el => {
            if (el) el.textContent = `Terasa seperti: ${Math.round(data.current.feelslike_c)}°`;
        });

        // Update humidity
        const humidityElements = document.querySelectorAll('#humidity, #humidityMobile');
        ...
        // Update wind speed
        const windElements = document.querySelectorAll('#windSpeed, #windSpeedMobile');
            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    try {
                        ...
                        this.hideLoading();
                        this.showError('Gagal mengambil data cuaca untuk lokasi Anda. Silakan cari kota secara manual.');
                    }
                },
                async (error) => {
                    console.warn('Geolocation failed:', error);
                    this.hideLoading();
                    this.showError('Gagal mendeteksi lokasi. Silakan cari kota secara manual.');
                },
                {
